...
from app import db
from app.caching.service import get_cache_teachers, get_cache_all_subjects, get_cache_subject_record, delete_cache
from app.school.subjects.forms import ExtraSubjectForm, EditExtraSubjectForm
from app.school.subjects.service import add_new_subject, handle_subject_edit
...
def subjects():
    if current_user.rights in ["admin", "user", "teacher"]:
        all_subjects = get_cache_all_subjects()
...
def add_subject():
    if current_user.rights in ["admin", "user"]:
        ...
        all_teachers = get_cache_teachers()
            ...
            try:
                if form.validate_on_submit():
                    ...
                    if new_subject:
                        ...
                        db.session.commit()
                        delete_cache(['all_subjects', 'subscription_subjects'])
...
def subject(subject_id, month_index):
    if current_user.rights in ["admin", "user", "teacher"]:
        ...
        subject_records, datetimes, subject_students, month = get_cache_subject_record(subject_id, month_index).values()
        all_subjects = get_cache_all_subjects()
