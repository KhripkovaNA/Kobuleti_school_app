...
# print(*[(lesson.date, lesson.start_time) for lesson in subject_lesson], sep='\n')


# for lesson in lessons_list:
#     new_lesson = lesson.copy()
#     new_lesson['date'] = (datetime.strptime(new_lesson['date'], '%d.%m.%Y').date()
#                           + timedelta(days=7)).strftime('%d.%m.%Y')
#     add_lesson(new_lesson)
param = -2
if param < 0:
    subject_lessons = Lesson.query.filter(Lesson.date < today,
                                          Lesson.subject_id == 12). \
    ...
    order_param = param
subject_lesson = subject_lessons[order_param] if subject_lessons and order_param < len(subject_lessons) else None

# if subject_lesson:
#     print(subject_lesson.date)
#     print([lesson.date for lesson in subject_lessons])
#
# else:
#     print('Nope')
# alina = Person.query.filter_by(id=1).first()
# mama = Person.query.filter_by(id=2).first()
# # print(mama in alina.parents)
# lesson_subject = Subject.query.filter_by(id=11).first()
...
# print(len(subject_lesson.students_registered.all()))
# print('Yeee!')

# lesson.id, lesson_type, subject.id, subject.subject_type, teacher.id
...
# def copy_lessons_to_next_week(previous_week_start_date, previous_week_end_date):
#     # Step 1: Identify lessons from the previous week
#     previous_week_lessons = Lesson.query.filter(
#         Lesson.date >= previous_week_start_date,
#         Lesson.date <= previous_week_end_date
#     ).all()
#
#     # Step 2 and 3: Create and add new lessons for the next week
#     next_week_lessons = []
#     for lesson in previous_week_lessons:
#         # Calculate the new date for the next week
#         new_date = lesson.date + timedelta(weeks=1)
#
#         # Create a new lesson with the same details but a new date
#         new_lesson = Lesson(
#             date=new_date,
#             start_time=lesson.start_time,
#             end_time=lesson.end_time,
#             lesson_completed=False,  # You may adjust this based on your requirements
...
#         next_week_lessons.append(new_lesson)
#
#     # Add the new lessons to the database
#     db.session.add_all(next_week_lessons)
#     db.session.commit()

# day_of_week_date = today - timedelta(days=today.weekday()) + timedelta(days=day_of_week) + week * timedelta(week=1)
def get_date(week, day_of_week):
    today = datetime.now().date()
...
start_date = get_date(0, 0)
end_date = get_date(0, 6)
